...
1. Built ELKS using ROM boot support
2. Placed kernel and filesystem at segment `C000`
3. Adjusted kernel text and data segments to avoid overwriting BIOS and video memory
4. Used BIOS console output instead of a direct console driver
...
1. Is the omission of `init_bh(TIMER_BH, timer_bh)` in the `CONFIG_TIMER_INT1C` path intentional?
2. Has anyone attempted to boot ELKS on systems without a PIC or PIT?
3. Are additional timing or scheduling issues expected when using `INT 1Ch` instead of IRQ0?
